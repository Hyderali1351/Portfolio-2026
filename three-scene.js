// three-scene.js — WebGL 3D hero background
// Uses global THREE (UMD build loaded via <script> tag before this file)

(function () {
  'use strict';

  // Kill the spider web canvas — Three.js handles the background from here
  window.__killSpiderWeb = true;
  const webCanvas = document.getElementById('web-canvas');
  if (webCanvas) webCanvas.style.display = 'none';

  // Stop the 2D aurora loop and hide its canvas
  window.__stopAurora = true;
  const auroraEl = document.getElementById('bg-aurora');
  if (auroraEl) auroraEl.style.opacity = '0';

  // Create a fresh canvas — browsers block WebGL on a canvas that already
  // has a 2D context, so we cannot reuse bg-aurora's canvas element.
  const canvas = document.createElement('canvas');
  Object.assign(canvas.style, {
    position: 'fixed', inset: '0', width: '100%', height: '100%',
    zIndex: '0', pointerEvents: 'none', display: 'block',
  });
  // Insert right after aurora so the spider-web canvas stays above (but it's now hidden)
  ...
  // ── Node factory ───────────────────────────────────────────────
  const ACCENT = ['#7c3aed', '#2563eb', '#be185d', '#0891b2', '#4338ca'];
  ...
  const pktMat = new THREE.MeshBasicMaterial({ color: '#00ff41' });
  ...
  // ── Animation loop — capped at 30 fps to save GPU headroom ────
  ...
  (function tick(ts) {
    ...
    // 30 fps cap: skip frames faster than ~33 ms
    ...
    // Float + slow rotate + pulse emissive
    nodes.forEach((n, i) => {
      const d = n.userData;
